/**
 * Cold War Grid Button Component
 * Button with animated grid pattern background
 */
...
export interface ColdWarGridButtonProps {
...
export const ColdWarGridButton: React.FC<ColdWarGridButtonProps> = ({
  children,
  variant = 'primary',
  size = 'md',
  theme = 'perseus',
  ...
  const gridStyle: CSSProperties = {
    ...
    bottom: 0,
    backgroundImage: `
      linear-gradient(0deg, rgba(${rgb}, 0.1) 1px, transparent 1px),
      linear-gradient(90deg, rgba(${rgb}, 0.1) 1px, transparent 1px)
    `,
  ...
  if (gridAnimated && !disabled) {
    gridStyle.animation = 'grid-move 2s linear infinite';
  }
...
export default ColdWarGridButton;
