...
export interface ScrollingTextLoaderProps {
  text?: string;
  color?: string;
  size?: number;
  className?: string;
...
const ScrollingTextLoader: React.FC<ScrollingTextLoaderProps> = ({
  text = 'Loading',
  color = '#ffffff',
  size = 64,
  className,
  ...
  return (
    <StyledWrapper className={className} $size={size} $rgbColor={rgbColor}>
...
const StyledWrapper = styled.div<{ $size: number; $rgbColor: string }>`
  .loader {
    --main-size: ${(props) => props.$size}px;
    --text-color: rgb(${(props) => props.$rgbColor});
    --shine-color: rgba(${(props) => props.$rgbColor}, 0.25);
    --shadow-color: #aaaaaa;
    display: flex;
...
export default ScrollingTextLoader;
