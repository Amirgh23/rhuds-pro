...
export interface ColdWarSliderProps {
  label?: string;
  value?: number;
  min?: number;
  max?: number;
...
export const ColdWarSlider: React.FC<ColdWarSliderProps> = ({
  label,
  ...
  const sizeMap = {
    ...
    md: { height: '6px', fontSize: '14px' },
    lg: { height: '8px', fontSize: '16px' },
  };
      ...
      <div style={{ position: 'relative', width: '100%', height: sizeMap[size].height }}>
        ...
        <div
          style={{
            ...
            boxShadow: glow && !disabled ? `0 0 10px rgba(${rgb}, 0.6)` : 'none',
          }}
        />
        <input
          type="range"
