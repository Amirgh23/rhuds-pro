...
export const HudSelect: React.FC<HudSelectProps> = ({
  ...
  return (
    ...
    >
      {options.map((option) => (
        <option key={option.value} value={option.value} style={{ backgroundColor: '#0A1225', color: '#fff' }}>
          {option.label}
        </option>
      ))}
    </select>
...
export const HudSwitch: React.FC<HudSwitchProps> = ({
  ...
  return (
    <div className="form-check form-switch" style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.5rem' }}>
...
export const HudInputGroup: React.FC<HudInputGroupProps> = ({ prepend, append, children }) => {
  ...
  return (
    <div style={{ display: 'flex', width: '100%' }}>
      {prepend && (
        <span style={{ ...addonStyle, borderTopRightRadius: 0, borderBottomRightRadius: 0, borderRight: 0 }}>
          {prepend}
        </span>
      )}
      <div style={{ flex: 1 }}>{children}</div>
      {append && (
        <span style={{ ...addonStyle, borderTopLeftRadius: 0, borderBottomLeftRadius: 0, borderLeft: 0 }}>
          {append}
        </span>
      )}
    </div>
...
export const HudFormFeedback: React.FC<HudFormFeedbackProps> = ({ children, type = 'invalid', tooltip = false }) => {
  const color = type === 'invalid' ? '#dc3545' : '#28a745';
...
export const HudInputValidated: React.FC<HudInputValidatedProps> = ({
  ...
  const borderColor = isInvalid ? '#dc3545' : isValid ? '#28a745' : COLOR;
