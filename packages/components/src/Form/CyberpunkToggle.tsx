...
export interface CyberpunkToggleProps {
  checked?: boolean;
...
const CyberpunkToggle: React.FC<CyberpunkToggleProps> = ({
  checked = false,
  onChange,
  color = '#03e9f4',
  label,
  ...
  return (
    <StyledWrapper $color={color} $rgb={rgb} className={className}>
      ...
      {label && <div className="cyber-toggle-label-text">{label}</div>}
    </StyledWrapper>
  );
};
...
const StyledWrapper = styled.div<StyledWrapperProps>`
  ...
  .cyber-toggle-particle {
    ...
    box-shadow: 0 0 4px ${(props) => `rgba(${props.$rgb.r}, ${props.$rgb.g}, ${props.$rgb.b}, 0.8)`};
  }
  ...
  .cyber-toggle-checkbox:focus + .cyber-toggle::after {
    ...
    inset: -4px;
    border-radius: 20px;
    border: 2px solid ${(props) => `rgba(${props.$rgb.r}, ${props.$rgb.g}, ${props.$rgb.b}, 0.5)`};
    opacity: 0.5;
  }
