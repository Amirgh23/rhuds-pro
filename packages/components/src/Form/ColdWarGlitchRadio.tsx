import React, { CSSProperties, useState } from 'react';
import { getRgbString, generateTechCode, ThemeVariant } from '../utils/coldWarUtils';

export interface ColdWarGlitchRadioProps {
  label?: string;
  name: string;
  checked?: boolean;
  onChange?: (checked: boolean) => void;
  size?: 'sm' | 'md' | 'lg';
  theme?: ThemeVariant;
  glow?: boolean;
  disabled?: boolean;
  className?: string;
  style?: CSSProperties;
}

const THEME_COLORS = {
  perseus: { primary: '#FFB000', background: '#0a0a0c', surface: '#1a1a1e' },
  greenTerminal: { primary: '#33FF00', background: '#0a0a0c', surface: '#1a1a1e' },
  satelliteView: { primary: '#00CCFF', background: '#3a3a3e', surface: '#2a2a2e' },
};

export const ColdWarGlitchRadio: React.FC<ColdWarGlitchRadioProps> = ({
  label,
  name,
  checked: controlledChecked,
  onChange,
  size = 'md',
  theme = 'perseus',
  glow = true,
  disabled = false,
  className = '',
  style = {},
}) => {
  const [internalChecked, setInternalChecked] = useState(false);
  const [glitchOffset, setGlitchOffset] = useState({ x: 0, y: 0 });
  const checked = controlledChecked !== undefined ? controlledChecked : internalChecked;
  const themeColors = THEME_COLORS[theme];
  const rgb = getRgbString(themeColors.primary);
  const sizeMap = {
    sm: { size: '16px', fontSize: '12px', dotSize: '8px' },
    md: { size: '20px', fontSize: '14px', dotSize: '10px' },
    lg: { size: '24px', fontSize: '16px', dotSize: '12px' },
  };

  React.useEffect(() => {
    if (checked && !disabled) {
      const interval = setInterval(
        () => setGlitchOffset({ x: (Math.random() - 0.5) * 2, y: (Math.random() - 0.5) * 2 }),
        100
      );
      return () => clearInterval(interval);
    } else {
      setGlitchOffset({ x: 0, y: 0 });
    }
  }, [checked, disabled]);

  const handleChange = () => {
    if (!disabled) {
      const newChecked = !checked;
      if (controlledChecked === undefined) setInternalChecked(newChecked);
      onChange?.(newChecked);
    }
  };

  return (
    <label
      className={className}
      style={{
        display: 'inline-flex',
        alignItems: 'center',
        gap: '8px',
        cursor: disabled ? 'not-allowed' : 'pointer',
        ...style,
      }}
      onClick={handleChange}
    >
      <div
        style={{
          position: 'relative',
          width: sizeMap[size].size,
          height: sizeMap[size].size,
          backgroundColor: disabled ? themeColors.surface : themeColors.background,
          border: `2px solid ${disabled ? '#666' : checked ? themeColors.primary : '#666'}`,
          borderRadius: '50%',
          cursor: disabled ? 'not-allowed' : 'pointer',
          transition: 'all 200ms cubic-bezier(0.68, -0.55, 0.265, 1.55)',
          display: 'flex',
          alignItems: 'center',
          justifyContent: 'center',
          boxShadow: checked && glow && !disabled ? `0 0 15px rgba(${rgb}, 0.6)` : 'none',
          transform: `translate(${glitchOffset.x}px, ${glitchOffset.y}px)`,
        }}
      >
        {checked && (
          <div
            style={{
              width: sizeMap[size].dotSize,
              height: sizeMap[size].dotSize,
              backgroundColor: themeColors.primary,
              borderRadius: '50%',
              transform: checked ? 'scale(1)' : 'scale(0)',
              transition: 'transform 200ms cubic-bezier(0.68, -0.55, 0.265, 1.55)',
            }}
          />
        )}
      </div>
      {label && (
        <span
          style={{
            fontSize: sizeMap[size].fontSize,
            fontFamily: "'Share Tech Mono', monospace",
            fontWeight: 500,
            textTransform: 'uppercase',
            letterSpacing: '0.03em',
            color: disabled ? '#666' : '#fff',
            userSelect: 'none',
          }}
        >
          {label}
        </span>
      )}
      <input
        type="radio"
        name={name}
        checked={checked}
        disabled={disabled}
        onChange={() => {}}
        style={{ display: 'none' }}
      />
    </label>
  );
};

export default ColdWarGlitchRadio;
